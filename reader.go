...
func validDelim(r rune) bool {
	return r != 0 && r != runeReadQuoteChar && r != '\r' && r != '\n' && utf8.ValidRune(r) && r != utf8.RuneError
}
...
parseField:
	for {
		...
		if len(line) == 0 || line[0] != readQuoteChar {
			...
			if !r.LazyQuotes {
				if j := bytes.IndexByte(field, readQuoteChar); j >= 0 {
			...
			line = line[quoteLen:]
			for {
				i := bytes.IndexByte(line, readQuoteChar)
				if i >= 0 {
					// Hit next quote.
					r.recordBuffer = append(r.recordBuffer, line[:i]...)
					line = line[i+quoteLen:]
					switch rn := nextRune(line); {
					case rn == runeReadQuoteChar:
						// `""` sequence (append quote).
						r.recordBuffer = append(r.recordBuffer, readQuoteChar)
						line = line[quoteLen:]
					case rn == r.Comma:
